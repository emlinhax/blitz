...
__forceinline size_t _wcstombs(char* mbstr, const wchar_t* wcstr, size_t count) {
    ...
    for (i = 0; i < count && wcstr[i] != L'\0'; ++i) {
        ...
        if (mbstr != NULL) mbstr[i] = (char)wcstr[i];
    }
    if (mbstr != NULL && i < count) mbstr[i] = '\0';
    return i;
}
...
namespace blitz {
    ...
    __forceinline u64 resolve_module(u64 crc)
    {
        ...
        for (PLIST_ENTRY curr = head->Flink; curr != head; curr = curr->Flink) {
            ...
            UNICODE_STRING* dll_base_name = (UNICODE_STRING*)(entry->Reserved4);
    ...
    __forceinline u64 resolve_export(u64 moduleptr, u64 crc)
    {
        ...
        IMAGE_EXPORT_DIRECTORY* export_directory = reinterpret_cast<IMAGE_EXPORT_DIRECTORY*>(moduleptr + ((IMAGE_OPTIONAL_HEADER)nt_header->OptionalHeader).DataDirectory[IMAGE_DIRECTORY_ENTRY_EXPORT].VirtualAddress);
...
#define BLITZ_FIND_MODULE(module_name) blitz::resolve_module(crc32(module_name))
#define BLITZ_FIND_EXPORT(module_name, export_name) blitz::resolve_export(blitz::resolve_module(crc32(module_name)), crc32(export_name))
#define BLITZ_FUNC(mod, name, type) using BLITZ_##name = type; BLITZ_##name name; name = reinterpret_cast< type > (BLITZ_FIND_EXPORT(mod, #name));
